...
pub fn part2<R: BufRead>(reader: R) -> Result<usize, Box<dyn std::error::Error>> {
    let game_scores: HashMap<usize, usize> = reader
        .lines()
    ...
    let mut acc = 0;
    for i in 1..game_scores.len() + 1 {
        acc += recurse(&game_scores, i);
    }

    Ok(acc)
}

fn recurse(map: &HashMap<usize, usize>, x: usize) -> usize {
    let score = map.get(&x).unwrap();
    let mut acc = 1;
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11"#;
    ...
    #[test]
    fn example_partial_1() {
        let input = r#"Card 1: 1 2 | 1 2
Card 2: 13 | 13
Card 3:  1 | 2 3 4"#;
        let expected = 3 + 2 + 1 + 1;
